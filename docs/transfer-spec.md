...
payload 双端均为**流式处理**，不在内存中拼装完整报文 —— 故支持至 4GiB 的大文件：
...
协议 `payloadSize` 为 u64（Python `struct "<Q"` / ArkTS 双 u32 拼接），此限制是**策略性**
的 —— 用于挡住误操作（如误推整个镜像），而非技术瓶颈。双端流式处理，
内存占用与文件大小无关；设备剩余空间不足时在写盘中 ENOSPC 失败并清理 `.part`。
